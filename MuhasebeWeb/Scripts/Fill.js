    ...
    switch (i) {
        case 0:
            ...
            if (branchGroups.length != 0) {
                ...
                $.each(branchGroups, function (index, item) {
                    $.ajax({
                        dataType: 'JSON',
                        type: 'GET',
                        url: '../Home/BranchJson',
                        ...
                        success: function (data) {
                            ...
                            $.each(data, function (index, d) {
                                $('#branchSelector').multiSelect('addOption', { value: d.id, text: d.name, index: 0 });
                            })
        ...
        case 2:
            if (companyTypes.length != 0) {
                ...
                $.each(companyTypes, function (index, item) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $.each(data, function (index, d) {
                                $('#companySelector').multiSelect('addOption', { value: d.id, text: d.name, index: 0 });
                            })
                        ...
                        if (data[0] == 0) {
                            ...
                            $.each(data, function (index, item) {
                                if (mindexer != item.indexer) {
                                    ...
                                    table = '<label>' + item.calcType + '</label></br><div class="row table-responsive"><table class="table table-bordered scrolldiv"><tr><th>#</th>';
                                    $.each(dates, function (index, tdate) {
                                        table += '<th>' + tdate + '</th>';
                                    })
                            ...
                            table += '</tr></table></br><div class="btn-group col-lg-12"> <button class="btn btn-success" onclick="newQuery();">Yeni Sorgu</button></div>';
                            $('#calculateresult').append(table);
                            ('.scrolldiv').scrollWidth;
