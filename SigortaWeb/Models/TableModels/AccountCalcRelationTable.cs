...
{
    public class AccountCalcRelationTable : BasicTable
    {
        public List<AccountCalcRelation> RowList { get; set; }

        public List<string> HeaderList { get; set; }

        public AccountCalcRelationTable(BasicTable T)
        {
            SetTable(T);
        }
        ...
        public override MvcHtmlString GetTable(int id)
        {
            if (id == 6)
            {
                ...
                RowList = Context.AccountCalcRelation.Take(20).ToList();
                ...
                foreach (AccountCalcRelation item in RowList)
                {
                    tableBuilder.InnerHtml += "<tr>";
                    ...
                    tableBuilder.InnerHtml += "<td id='acr-" + item.GetType().GetProperty("id")?.GetValue(item) + "'><i onclick='Update(this)'; class='fa fa-edit' style='cursor:pointer'></i> </td>";
                    tableBuilder.InnerHtml += "<td id='acr-" + item.GetType().GetProperty("id")?.GetValue(item) + "'><i onclick='Remove(this)'; class='fa fa-remove' style='color:red;cursor:pointer'></i> </td>";
                    tableBuilder.InnerHtml += "</tr>";
                }
                tableBuilder.InnerHtml += "</tbody>";
                ...
                return MvcHtmlString.Create("<div class='row'><input type='text' class='form-control col-lg-8' id='TbSearch' placeholder='Arama için Yazınız'><input type='text' class='form-control col-lg-2' id='TbFilter' placeholder='Kriter Yazınız'> <button id='BtnSearch' data-id='acr' class='btn btn-info col-lg-2'>Ara</button></div>" + tableBuilder.ToString(TagRenderMode.Normal) + "<br><br><div class='text-center'><button id='BtnNext' data-id='ac' class='btn btn-lg btn-success'>Devamını Gör</button></div><br><br><br>");
            }
        ...
        public override bool RemoveData(int id)
        {
            try
            {
                ...
                Context.AccountCalcRelation.Remove(Context.AccountCalcRelation.Find(id));
                return true;
            }
            catch (Exception e)
            {
        ...
        private void FillPropNames(AccountCalcRelation T)
        {
            if (T != null)
            {
