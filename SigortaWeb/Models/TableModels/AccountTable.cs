...
{
    public class AccountTable : BasicTable
    {

        public List<Account> RowList { get; set; }

        public List<string> HeaderList { get; set; }

        public AccountTable(BasicTable T)
        {
            SetTable(T);
        }
        ...
        public override MvcHtmlString GetTable(int id)
        {
            if (id == 4)
            {
                ...
                RowList = Context.Accounts.Take(20).ToList();
                ...
                foreach (Account item in RowList)
                {
                    tableBuilder.InnerHtml += "<tr>";
                    ...
                    tableBuilder.InnerHtml += "<td id='a-" + item.GetType().GetProperty("id")?.GetValue(item) + "'><i onclick='Update(this)'; class='fa fa-edit' style='cursor:pointer'></i> </td>";
                    tableBuilder.InnerHtml += "<td id='a-" + item.GetType().GetProperty("id")?.GetValue(item) + "'><i onclick='Remove(this)'; class='fa fa-remove' style='color:red;cursor:pointer'></i> </td>";
                    tableBuilder.InnerHtml += "</tr>";
                }
                tableBuilder.InnerHtml += "</tbody>";
                ...
                return MvcHtmlString.Create("<div class='row'><input type='text' class='form-control col-lg-8' id='TbSearch' placeholder='Arama için Yazınız'><input type='text' class='form-control col-lg-2' id='TbFilter' placeholder='Kriter Yazınız'><button id='BtnSearch' data-id='a' class='btn btn-info col-lg-2'>Ara</button></div>" + tableBuilder.ToString(TagRenderMode.Normal)+ "<br><br><div class='text-center'><button id='BtnNext' data-id='a' class='btn btn-lg btn-success'> Devamını Gör</button></div><br><br><br>");
            }
        ...
        private void FillPropNames(Account T)
        {
            if (T != null)
            {
