...
#pragma warning restore CS0472 // The result of the expression is always 'true' since a value of type 'int' is never equal to 'null' of type 'int?'
            {
                List<Company> cmp = Context.Companies.Where(s => s.companyTypeId == id).ToList().OrderByDescending(s => s.name).ToList();
        ...
        {
            ViewBag.Sayac= Context.Sayacs.FirstOrDefault();
            return View(Context.Tables.FirstOrDefault());
        }
